...
void CardanoMethod(const T *coef, std::complex<T> *results) {
  if((coef==nullptr) || (results == nullptr))
    throw real_gas_models::modelExceptions("Cardano method. coef or result == nullptr\n");
  if (coef[0]==0.0)
    throw real_gas_models::modelExceptions("Cardano method. Get eqution > 3 power\n");
  ...
  if(S>0.00001) {       //checked
      T temp=std::acos( R/std::pow(Q,1.5))/3.0;
      results[0]=-2.0*std::sqrt(Q)*std::cos(temp) -b/3.0 ;
      results[1]=-2.0*std::sqrt(Q)*std::cos(2.094395 +temp) -b/3.0;       // 2*PI/3 = 2.094395
      ...
      if(Q>0.0) {
          ...
          if (x<1.0) {
              throw real_gas_models::modelExceptions("Cardano method. Method have error (S<0) (Q>0)\n");
            }
          ...
          results[2]=results[1] - std::complex<T>(0.0,1.0)*std::sqrt((T)3.0*Q)*std::sinh(arc);
          results[1]+=std::complex<T>(0.0,1.0)*std::sqrt((T)3.0*Q)*std::sinh(arc);
        }
      else if(Q<0.0) {    // checked
          T x=std::abs(R/std::pow(-Q,1.5));
          T arc = std::log(x+std::sqrt(x*x+1.0))/3.0;
          results[0]=-2.0*signR*std::sqrt(-Q)*std::sinh(arc) - b/3.0;
          results[1]=signR*std::sqrt(-Q)*std::sinh(arc) - b/3.0;
          results[2]=results[1] - std::complex<T>(0.0,1.0)*std::sqrt((T)3.0*(-Q))*std::cosh(arc);
          results[1]+=std::complex<T>(0.0,1.0)*std::sqrt((T)3.0*(-Q))*std::cosh(arc);
        }
      if(Q==0){           //not checked
...
bool CardanoMethod_HASUNIQROOT(const T *coef, T *results) {
  if((coef==nullptr) || (results == nullptr))
    throw real_gas_models::modelExceptions("Cardano method. coef or result == nullptr\n");
  if (coef==results)
    throw real_gas_models::modelExceptions("Cardano method. *result must != coef\n");
  std::vector<std::complex<T>> vec(3);
  CardanoMethod(coef,&vec[0]);
  results[0]=std::real(vec[0]);
  if (std::imag(vec[1])==0.0) {
