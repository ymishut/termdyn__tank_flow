...
double real_gas_models::lineIntegrateRK2::operator()(double t, double vLeft, double vRigth, double) {
...
void real_gas_models::initializeRK2::operator()(std::vector<double> &tempvec, double pi, double t,double) {
  tempvec[0] =  1.0;
  tempvec[1] =  -3.0*t/pi;
...
double real_gas_models::lineIntegratePR::operator()(double t, double vLeft, double vRigth,double ac) {
  const double J   = 3.253,
               alf = std::pow((1.0+(0.37464 + 1.54226 * ac-0.26992*ac*ac)*(1.0-std::sqrt(t))),2.0);
  return t*J*(std::log(std::abs(vRigth-0.25))-std::log(std::abs(vLeft-0.25))) +
      6.8448*alf*(std::log(std::abs((vRigth+0.60355)/(vRigth-0.10355))) - std::log(std::abs((vLeft+0.60355)/(vLeft-0.10355))));
}

void real_gas_models::initializePR::operator()(std::vector<double> &tempvec, double pi, double t,double ac) {
