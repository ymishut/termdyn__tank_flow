...
real_gas_models::Redlich_Kwong2::Redlich_Kwong2(modelName mn, std::shared_ptr<constgasparameters> &cgp)
  :modelGeneral::modelGeneral(mn,cgp) {
  modelCoefA_=0.42748*std::pow(parameters_->cgetR(),2.0)*std::pow(parameters_->cgetT_K(),2.5)/
      parameters_->cgetP_K();
  modelCoefB_=0.08664*parameters_->cgetR()*parameters_->cgetT_K()/parameters_->cgetP_K();
}
...
bool real_gas_models::Redlich_Kwong2::isValid() const {
  return (parameters_->cgetPressure()/parameters_->cgetP_K() < 0.5*parameters_->cgetTemperature()/parameters_->cgetT_K());
}
...
float real_gas_models::Redlich_Kwong2::getVolume(float p, float t) const {
  if ((p<=0.0)||(t<=0.0))
    throw modelExceptions("Redlich_Kwong2::setVolume bad input p: " + std::to_string(p) + " t: " + std::to_string(t));
  std::vector<float> coef{ 1.0,
                           -parameters_->cgetR()*t/p,
                               modelCoefA_/(p*std::sqrt(t))-parameters_->cgetR()*t*modelCoefB_/p-modelCoefB_*modelCoefB_,
...
#ifndef NDEBUG
    throw modelExceptions ("Redlich_Kwong::setTemperature calculation error #1");
#endif
  }
  if ((coef[4]<=0.0)||(!std::isfinite(coef[4])))
    throw modelExceptions ("Redlich_Kwong::setTemperature calculation error (m.b. input data is NaN)");
  return coef[4];
}
...
float real_gas_models::Redlich_Kwong2::getPressure(float v, float t) const {
  if ((v<=0.0)||(t<=0.0))
    throw modelExceptions("Redlich_Kwong2::setPressure bad input v: " + std::to_string(v) + " t: " + std::to_string(t));
  const float temp    = parameters_->cgetR()*t/(v-modelCoefB_) - modelCoefA_/(std::sqrt(t)*v*(v+modelCoefB_));
