...
real_gas_models::modelGeneral::modelGeneral(modelName mn, std::shared_ptr<constgasparameters> &cgp)
  :parameters_(std::unique_ptr <Igasparameters>(new gasparameters(0.0,0.0,0.0,cgp))),
    phasediag_model_(mn),
    bp_(phasediagram::getCalculated().getBinodalPoints(cgp->V_K,cgp->P_K,cgp->T_K,phasediag_model_,cgp->acentricfactor)) {
}
...
int real_gas_models::modelGeneral::setState_phasesub(float p) {
  ...
  return (bp_.p.end() - std::find_if(bp_.p.begin()+1,bp_.p.end(),std::bind2nd(std::less_equal<float>(),p)));
}
...
real_gas_models::state_phase real_gas_models::modelGeneral::setState_phase(float v, float p, float t) {
  ...
  if(!iter) {
#ifndef NDEBUG
      throw modelExceptions("real_gas_models::modelGeneral::setState_phase");
#endif
      std::cout<<" modelGeneral: gas have too low pressure\n";
      return ((v<=parameters_->cgetV_K()) ? state_phase::LIQ_STEAM : state_phase::GAS);
    }
  iter = bp_.p.size()-iter-1;
  const float p_path = (p-bp_.p[iter+1]) / (bp_.p[iter]-bp_.p[iter+1]);   // %
  if (v < parameters_->cgetV_K()) {                     // left branch of binodal
      const float vapprox = bp_.vLeft[iter]-(bp_.vLeft[iter+1]-bp_.vLeft[iter])*p_path;
      return ((v<vapprox) ? state_phase::LIQUID : state_phase::LIQ_STEAM );
    }
  const float vapprox = bp_.vRigth[iter]+(bp_.vRigth[iter+1]-bp_.vRigth[iter])*p_path;
