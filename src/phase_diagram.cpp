...
void real_gas_models::phasediagram::calculateBinodal(std::shared_ptr<binodalpoints>& bdp, modelName mn,
                                                                                     double acentric) {
  ...
  for (size_t t_iter=0;t_iter<nPoints;++t_iter) {
    ...
    //calculate derivate(dpressure/dtemperature)=0
      try{
        ...
        std::cout<<e.what() << " for temperature index:" << std::to_string(t_iter) << '\n';
        bdp->t[t_iter]=-1.0;
        continue;
      }
      ...
      while(true) {
...
#ifndef NDEBUG
              throw modelExceptions("Calculatebinodal() have problem 3, t with index "+std::to_string(t_iter));             //problem 3
#endif
              bdp->t[t_iter]=-1.0;
              break;
            }
          ...
          splineAR=integrateFun(bdp->t[t_iter],bdp->vLeft[t_iter],bdp->vRigth[t_iter],acentric);
...
void real_gas_models::phasediagram::checkResult(std::shared_ptr<binodalpoints> &bdp) {
...
void real_gas_models::phasediagram::eraseElements(std::shared_ptr<binodalpoints> &bdp, const size_t i) {
...
void real_gas_models::phasediagram::searchNegative(std::shared_ptr<binodalpoints> &bdp, std::deque<double> &v) {
  auto hasnegative =[](std::deque<double>& vec){
    return vec.end()-std::find_if( vec.begin(),vec.end(),std::bind2nd(std::less_equal<double>(),0.0));};
  int hasneg=1;
  int vecsize = v.size();
  while (true){
      if((hasneg=hasnegative(v))) {
#ifndef NDEBUG
          std::cout << " Some troubles in real_gas_models::phasediagram::checkResult.\n Bad calculation.\n";
#endif
...
real_gas_models::binodalpoints real_gas_models::phasediagram::getBinodalPoints(double VK, double PK,
                                                         double TK, modelName mn, double acentric) {
  bool checkinput = ((VK<=0.0)||(PK<=0.0)||(TK<=0.0)||(acentric<=0.0));
  checkinput = checkinput || ((!std::isfinite(VK))||(!std::isfinite(PK))||(!std::isfinite(TK))||
                                                                    (!std::isfinite(acentric)));
  if (checkinput)
    throw modelExceptions("phasediagram::getBinodalPoints get incorrect data: V_K, P_K, T_K \
  ...
  if(calculated_.find(um)==calculated_.end()){
      ...
      if (calculated_.find(um)==calculated_.end()){
          ...
          try{
            ...
            std::cout<< "OUT_OF_RANGE exception in phasediagram::calculateBinodal"<<std::endl;
            throw;
          } catch(modelExceptions& e){
  ...
  auto f = [] (std::deque<double> &vec, double K){
    std::transform(vec.begin(),vec.end(),vec.begin(),std::bind1st(std::multiplies<double>(),K));};
  f(bp.vLeft,VK);
  f(bp.vRigth,VK);
  f(bp.p,PK);
  f(bp.t,TK);
...
void real_gas_models::phasediagram::eraseBinodalPoints(modelName mn, double acentric) {
  std::lock_guard<std::mutex> lg(mtx);
...
bool real_gas_models::operator< (const phasediagram::uniqueMark& lum, const phasediagram::uniqueMark& rum) {
  return std::tie (lum.mn, lum.acentricfactor) < std::tuple<size_t,double>(rum.mn,rum.acentricfactor + 0.0001) &&
         std::tie (rum.mn, rum.acentricfactor) > std::tuple<size_t,double>(lum.mn,lum.acentricfactor+0.0001);
}

//bool real_gas_models::phasediagram::uniqueMark::operator<(const phasediagram::uniqueMark &um) const {
//  if(((this->acentricfactor+0.0001)>um.acentricfactor) &&
//     ((this->acentricfactor-0.0001)<um.acentricfactor))
//    return (this->mn) < um.mn;
//  return this->acentricfactor < um.acentricfactor;
//}
