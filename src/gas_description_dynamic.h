#ifndef GAS_DESCRIPTION_DYNAMIC
#define GAS_DESCRIPTION_DYNAMIC
...
namespace real_gas_models {
  ...
  class potentials {
    ...
    friend std::ostream& operator<< (std::ostream & outstream, const potentials &pt);
  ...
  public:
    gasparameters_dynamic(float v,float p,float t, std::shared_ptr<constgasparameters> cgp);
    gasparameters_dynamic(parameters prs, std::shared_ptr<constgasparameters> cgp);
  ...
  std::ostream& operator<< (std::ostream &outstream, const gasparameters_dynamic &gp);

}

#endif // GAS_DESCRIPTION_DYNAMIC
