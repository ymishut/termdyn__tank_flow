...
real_gas_models::balloonFlowDynamic::balloonFlowDynamic(const std::shared_ptr<modelGeneral> &spmg, gasparameters &outballoon,
                                                                                                            float V, float F)
  :calculateModel_(spmg),
    calculateModelConverter_(spmg.get()),
    outbl_(outballoon),
    bl_(V,F) {
  if ((V<=0.0)||(F<=0.0)||(!std::isfinite(F))||(!std::isfinite(V))) {
      throw modelExceptions("balloonFlowDynamic constructor get not correct args V or F");
    }
  accuracy_ = MODELS_CALC_ACCUR;                                              /// not mistake, i think
  parameters rmsM  = calculateModel_->getParametersCopy(),
             rmsOB = outbl_.cgetParameters();
  if (rmsOB.pressure == 0.0) {                                                                    /// not common case
      calculateModel_->setPressure(rmsOB.volume,rmsOB.temperature);                               /// calculate all outbl_ parameters
      outbl_.csetParameters(calculateModel_->getVolume(),calculateModel_->getPressure(),
                            calculateModel_->getTemperature(),calculateModel_->getState());
      ...
      throw modelExceptions("balloonFlowDynamic constructor get not correct args in outballoon.");
    }
}
...
template
void real_gas_models::balloonFlowDynamic::calculateFlow <class real_gas_models::DerivateFunctorInflow>(const float dtime,
                                                                            const size_t outRate, formatted_output &out);

template
void real_gas_models::balloonFlowDynamic::calculateFlow <class real_gas_models::DerivateFunctorOutflow>(const float dtime,
                                                                             const size_t outRate, formatted_output &out);

template <class DerivateFunctorType>
void real_gas_models::balloonFlowDynamic::calculateFlow (const float dtime, const size_t outRate, formatted_output &out) {
  out << "\n";
  ...
  while (pless<pmore) {
      if(!(step % outRate)) {
          out << time << calculateModel_->getPressure() << 1.0 / calculateModel_->getVolume()
              << calculateModel_->getTemperature() << stateToString[(size_t)calculateModel_->getState()] << "\n";
        }
      try {
      ...
      boost::numeric::odeint::integrate_n_steps(method, fdxdt,x,time,dtime,1,fdxdtOBS);
...
void real_gas_models::balloonFlowDynamic::updatePress(float &pm, float &pl, real_gas_models::DerivateFunctorInflow ) {
  (void)pm;
  pl = calculateModel_->getPressure();
}

void real_gas_models::balloonFlowDynamic::updatePress(float &pm, float &pl, real_gas_models::DerivateFunctorOutflow ) {
  (void)pl;
