...
namespace {
  std::map< std::string,int> equations {{"IDEAL_GAS",1},{"REDLICH_KWONG",2},{"PENG_ROBINSON",3}};
}
...
real_gas_models::modelGeneral *real_gas_models::Ideal_gas_equation::getCalculatingModel(
...
real_gas_models::modelGeneral *real_gas_models::Redlich_Kwong_equation::getCalculatingModel(
    modelName mn, std::shared_ptr<constgasparameters> &cgp) {
  try{
...
real_gas_models::modelGeneral *real_gas_models::Peng_Robinson_equation::getCalculatingModel(
    modelName mn, std::shared_ptr<constgasparameters> &cgp) {
  try {
...
real_gas_models::balloonFlowDynamic *real_gas_models::GasDynamic::getFlowCalculator (
                                    const std::shared_ptr<real_gas_models::modelGeneral> &spmg,
                                real_gas_models::gasparameters &outballoon, float V, float F) {
  if (spmg == nullptr) {
      std::cout << " Object FlowCalculator wasn't created. Constructor get nullptr to model\n";
      return nullptr;
    }
  try {
...
void real_gas_models::GasDynamic::calculation(std::shared_ptr<InputData> &idp) {
  ...
  std::shared_ptr<constgasparameters> cgp (constgasparameters::getInstance(idp_->getConstgasparameters()));
  if (eos==nullptr) {
  ...
  auto mg = std::shared_ptr<modelGeneral>(eos->getCalculatingModel(modelName::PENG_ROBINSON,cgp));
  if (mg==nullptr) {
  ...
  (InOutCheck == "IN") ? up->calculateFlow<DerivateFunctorInflow>(MODELS_DTIME,MODELS_STEPS,SOout)
       : ((InOutCheck == "OUT") ? up->calculateFlow<DerivateFunctorOutflow>(MODELS_DTIME,MODELS_STEPS,SOout)
                          : msgf());
}
  ...
  try {
    switch (func) {
      case 'V': {
          ...
          pgp = std::make_shared <gasparameters> (0.0,params[4],params[5],mg->getConstParameters());
          return getFlowCalculator(mg,*pgp,balloon.first,balloon.second);
        }
      case 'P': {
          mg->setPressure(params[0],params[2]);
          pgp = std::make_shared <gasparameters> (params[3],0.0,params[5],mg->getConstParameters());
          return getFlowCalculator(mg,*pgp,balloon.first,balloon.second);
        }
      default:
...
std::shared_ptr<real_gas_models::Equation_of_state> real_gas_models::GasDynamic::setEOS() {
  int eosch;
  std::shared_ptr<Equation_of_state> eos;
  try {
    eosch = equations.at(idp_->getEquation());
  } catch (std::exception &e) {
    std::cout<<idp_->getEquation() <<"\n Bad input cannot create Real_gas_equation creator! "<<std::endl;
    return eos;
  }
  switch (eosch) {
    case 1:
