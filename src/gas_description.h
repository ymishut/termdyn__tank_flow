#ifndef GAS_DESCRIPTION_H
#define GAS_DESCRIPTION_H
...
namespace real_gas_models {
  ...
  enum class state_phase{SCF=0,LIQUID,LIQ_STEAM,GAS};     // SCF: t>T_K, p>P_K;                       GAS: p_binodal < p < p_K, t>t_binodal;
  static const std::vector<std::string> stateToString {   // LIQUID: p<P_K; v<vleft;     
                                                      		//in perspective:  LIQ_STEAM: p<p_binodal, vleft < v < vrigth;
             "SCF","LIQUID","LIQ_STEAM","GAS"};           // There are not metastable states (between binodal and spinodal)
  ...
  struct constgasparameters: private boost::noncopyable {             // add link in shared_ptr
    const float  V_K,              // K point parameters (critical point)
                 ...
                 beta_kr,          //beta_kr=beta_kr(adiabatic_index) = [0.0, ... ,1.0]
                                              // if (pressure_1/pressure_2 < beta_kr): flow velocity = const, (maximum)
  ...
  public:
    static std::shared_ptr<constgasparameters> getInstance(float vk,float pk,float tk,float mol, float adi, float cv, float af);
    static std::shared_ptr<constgasparameters> getInstance(std::vector<float> vec);

  private:
    constgasparameters(float vk,float pk,float tk,float mol,float adi, float cv,float b,float R,float af);
  ...
  //================================
    ...
    public:
      ...
      virtual std::shared_ptr<constgasparameters> cgetConstparameters() const = 0;
